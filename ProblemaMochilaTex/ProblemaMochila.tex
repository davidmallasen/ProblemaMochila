...
El problema de la mochila es uno de los 21 problemas NP-completos de Richard Karp, lista elaborada en 1972 y perteneciente a su trabajo \textit{Reducibility Among Combinatorial Problems}. Esto surgió como profundización del trabajo de Stephen Cook, quien en 1971 había demostrado uno de los resultados más importantes y pioneros de la complejidad computacional: la NP-completitud del Problema de satisfacibilidad booleana (SAT). El descubrimiento de Karp de que todos estos importantes problemas eran NP-completos, motivó el estudio de la NP-completitud y la indagación en la famosa pregunta de si $P=NP$.
...
Definimos la función indicadora $x_i \in \{0, 1\}$ que representará si se ha cogido el objeto $i$ ($x_i = 1$) o no ($x_i = 0$). El problema consiste en maximizar \[\sum_{i=1}^{n} v_i x_i\] con la restricción de $\sum_{i=1}^{n} p_i x_i < M$. La solución del problema vendrá dada por el conjunto de las $x_i$, de las cuales podremos obtener también el valor óptimo.\\
...
\lstinputlisting[firstline=561, lastline=592]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\lstinputlisting[firstline=613, lastline=640]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\lstinputlisting[firstline=907, lastline=943]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\]
Así pues, vamos probando cada objeto y si no cabe no lo cogemos, pero si cabe tomamos el máximo entre cogerlo y no cogerlo. Para ello recorremos la tabla por filas de forma descendente (cada vez el intervalo $[0, i]$ es más grande) y cada fila la recorremos de izquierda a derecha (cada vez la mochila soporta un peso mayor $j$ hasta llegar a $M$). De esta forma el valor que buscamos lo tendremos en la posición $(n, M)$.
...
En este apartado implementaremos un algoritmo de ramificación y poda para resolver el problema de la mochila en la versión 0-1. Al igual que en programación dinámica, obtendremos un coste exponencial ($O(n2^n)$) aunque con las podas se conseguirá reducir esto de forma efectiva.
...
\lstinputlisting[firstline=137, lastline=255]{../ProblemaMochilaCpp/ProblemaMochila.cpp}

\subsubsection{Análisis de costes}

Analizaremos ahora los costes en tiempo y memoria del algoritmo. En cuanto al tiempo tendremos un coste en el caso peor $O(n2^n)$, donde $n$ es el número de objetos. Cada iteración del bucle tendrá un coste lineal en $n$ y el bucle se realizará en el caso peor $2^n$ veces. Esto lo podemos razonar desde la estructura de árbol binario que se va generando. Veremos más adelante en la sección \ref{sec:Comparacion} cómo afecta la poda al número de nodos explorados frente al número máximo de nodos que podremos tener. Se reducirá así el tiempo de ejecución del algoritmo tanto más cuanta más poda hagamos.\\ 

En cuanto al coste en espacio tenemos un coste lineal en $n$ por cada nodo y a lo sumo $2^{n-1}$ nodos ya que cada vez vamos sacando uno de la cola de prioridad y hay como mucho $2^{n-1}$ hojas en cada momento. Luego el coste en memoria es también de $O(n2^n)$.

\subsection{Algoritmo genético} \label{sec:algGen}

Implementaremos ahora un algoritmo genético para resolver el problema de la mochila 0-1. Al tratarse de un algoritmo heurístico no se asegura una solución óptima aunque generalmente obtendremos una buena solución en un tiempo razonable. Las ideas expuestas en esta sección son mayoritariamente propias, salvo las referencias a \cite{hrist-shre04}. Los conocimientos previos para llegar al algoritmo vienen de un estudio anterior basado en \cite{micha95}, \cite{mitch99} y \cite{neap15} capítulo 10.
...
\lstinputlisting[firstline=259, lastline=278]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\lstinputlisting[firstline=291, lastline=342]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\lstinputlisting[firstline=389, lastline=435]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\lstinputlisting[firstline=437, lastline=469]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\lstinputlisting[firstline=471, lastline=509]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\lstinputlisting[firstline=511, lastline=557]{../ProblemaMochilaCpp/ProblemaMochila.cpp}
...
\section{Comparación} \label{sec:Comparacion}

Comparemos ahora los distintos algoritmos que hemos implementado ejecutando una batería de casos de prueba según hemos construido en el apartado \ref{sec:desCasosPrueba}. Construiremos tres casos de prueba para cada número de objetos y los ejecutaremos 3 veces cada uno con los distintos algoritmos, salvo en el caso del algoritmo genético. Para este algoritmo, con el fin de poder estudiar mejor su comportamiento, ejecutaremos cada caso 10 veces. El peso y el valor máximo de cada objeto será 100 y el peso máximo soportado por la mochila será 2500. En el caso de programación dinámica con 1000000 objetos, por cuestiones de memoria, reducimos el tamaño máximo de la mochila a 1000. Estos valores son completamente arbitrarios ya que, salvo casos extremos y en programación dinámica, el comportamiento de los algoritmos es independiente de estos factores y la relación entre ellos.\\

Generaremos casos de $10^3$, $10^5$, $10^6$, $10^7$ y $10^8$ objetos. Probaremos el algoritmo voraz con todas las cantidades, los algoritmos de ramificación y poda y programación dinámica solo con hasta $10^6$ objetos y el algoritmo genético solo con 1000 objetos. Estos números y el hecho de ejecutar los algoritmos solo para algunos tamaños de casos de prueba lo hemos deducido empíricamente viendo los tiempos que tardaba cada algoritmo.\\

Mostramos a continuación en la tabla \ref{tab:casosPruebaVorYRamPodaR} los datos recopilados en diferentes ejecuciones de voraz y ramificación y poda. Los tiempos que se muestran son la media de las tres ejecuciones de cada caso.
...
\begin{table}[H]
	...
	\end{tabular}
	\caption{Tiempos (en segundos), valores y nodos explorados de las ejecuciones de los distintos casos de prueba sobre los algoritmos voraz y ramificación y poda en su variante con números reales.}
	\label{tab:casosPruebaVorYRamPodaR}
\end{table}

Observamos que, como era de esperar, el algoritmo voraz es el más rápido con diferencia e incluso tomando tamaños de $10^8$ elementos tarda menos de un minuto. En comparación tenemos el algoritmo de ramificación y poda cuando trabaja con números reales (veremos a continuación por qué diferenciamos). Este algoritmo exponencial para el caso de $10^6$ elementos tarda entre dos horas y tres cuartos y tres horas y cuarto dependiendo del caso en particular. Esta diferencia considerable entre unos casos (100000A y 100000C) y otros (100000B) del mismo tamaño la podemos achacar a la cantidad de ramas que podrá podar el algoritmo en su ejecución. Como podemos observar, en el caso en que tarda más explora unos 90000 nodos más que en los otros dos.\\
...
\begin{table}[h]
	...
	\end{tabular}
	\caption{Parámetros para las dos ejecuciones del algoritmo genético. Los nombres se corresponden con los del código descrito en la sección \ref{sec:algGen}.}
	\label{tab:paramGen}
\end{table}
